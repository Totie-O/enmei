...
## 商品资料
df_product = pd.read_excel(file_product[0], usecols=["图片","款式编码","商品编码","颜色","规格","年份","季节",'分类','创建时间'])
...
df_product_merged = df_product_merged.loc[:, ['款式编码','商品编码','颜色','图片','规格','年份','季节','分类','新品类（企划版）','创建时间']]

# SPU
df_product_merged_SPU = df_product_merged.loc[:, ['款式编码','图片','年份','季节','分类','新品类（企划版）','创建时间']]
...
df_product_merged_SKC = df_product_merged.loc[:, ['款色', '款式编码','颜色','图片','年份','季节','分类','新品类（企划版）','创建时间']]

df_product_merged_SKC = df_product_merged_SKC.drop_duplicates(subset=['款色'])
# SKU
df_product_merged_SKU = df_product_merged.loc[:, ['商品编码', '款色', '款式编码','图片','颜色','规格','年份','季节','分类','新品类（企划版）','创建时间']]
